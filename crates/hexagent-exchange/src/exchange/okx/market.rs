//! OKX WebSocket market feed — 5-level orderbook depth (books5 channel, 100ms).
...
async fn okx_ws_task(
    ...
    loop {
        ...
        if let Err(e) = write.send(Message::Text(sub.to_string())).await {
            warn!("[OKX] subscribe failed: {}", e);
            continue;
        }
        info!("[OKX] Connected, subscribed to {:?}", symbols);
        ...
        loop {
            tokio::select! {
                ...
                _ = ping_interval.tick() => {
                    // OKX accepts text "ping" for keepalive
                    if let Err(e) = write.send(Message::Text("ping".to_string())).await {
                        warn!("[OKX] Ping send failed: {}", e);
                        break;
                    }
                }
                read_result = tokio::time::timeout(STALE_THRESHOLD, read.next()) => {
                    let msg = match read_result {
                        Ok(Some(Ok(m))) => m,
                        Ok(Some(Err(e))) => { warn!("[OKX] WS read error: {}", e); break; }
                        Ok(None) => { warn!("[OKX] WS closed"); break; }
                        Err(_elapsed) => {
                            warn!("[OKX] No message for {:.0}s (stall watchdog) — reconnecting",
    ...
    info!("[OKX] WS task exiting");
}

impl ExchangeMarket for OkxMarket {
    fn connect(&mut self) -> Result<()> {
        ...
        crate::async_rt::handle().spawn(okx_ws_task(symbols, event_tx, shutdown));
        Ok(())
    }
    ...
    fn next_event(&mut self) -> Result<Option<MarketEvent>> {
        ...
        match rx.try_recv() {
            ...
            Err(crossbeam_channel::TryRecvError::Disconnected) => {
                Err(anyhow!("OKX WS task ended unexpectedly"))
            }
        }
    }
