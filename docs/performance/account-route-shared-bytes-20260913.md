...
Validation includes held-snapshot byte sharing, owner rebind and stale-owner deletion, four concurrent owners publishing 400 batches without lost routes, and the full account suite (251 passed, eight opt-in benchmarks ignored). Existing ordering, replay/idempotence, queue overflow and instance-isolation tests remain enabled. An exchange replay test hit its existing two-second debug barrier under simultaneous compilation; it passed in isolation without a timeout change. See PR validation for the subsequent full serial result.
